...
from .fsl.meta_mnist import Mnist2Fashion, Fashion2Mnist
...
from .classification.cifar10 import Cifar10
from .classification.cifar100 import Cifar100
...
class DatasetBuilder:
    ...
    def load_dataset(dataset_config: DatasetConfig) -> Union[DatasetWrapper, Dataset]:
        ...
        elif dataset_config.dataset_type == "mnist2fashion":
            Logger.instance().debug("Loading dataset Mnist2Fashion (type FewShotDataset)")
            return Mnist2Fashion(dataset_config)
        elif dataset_config.dataset_type == "fashion2mnist":
            Logger.instance().debug("Loading dataset Fashion2Mnist (type Mnist2Fashion)")
            return Fashion2Mnist(dataset_config)
        elif dataset_config.dataset_type == "cifar100":
            Logger.instance().debug("Loading dataset Cifar100 (type DatasetCls)")
            return Cifar100(dataset_config)
        elif dataset_config.dataset_type == "cifar10":
            Logger.instance().debug("Loading dataset Cifar10 (type DatasetCls)")
            return Cifar10(dataset_config)
        else:
            raise ValueError(
                ...
                "`meta_album_cls`, `mnist`, `fashion_mnist`, `mnist2fashion`, `fashion2mnist`, `cifar100`, `cifar10` " +
                "for dataset_type.\n" +
